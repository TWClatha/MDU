...
SELECT DISTINCT prms_key,cstm_fld_3_txt,cstm_fld_10_txt,
       da.end_day_key as mo_end
...
SELECT cust_acct_key,prod_cls_mtrx_key,prod_srvc_key,da.end_day_key as mo_end,
...
SELECT PRM.PRMS_KEY, PRM.MO_END, PRM.MO_NM,PRM.LVL_2_NM as REGION,
       ...
       CA.NET_RVNU_AMT,
       PRM.ORG_KEY,
       CA.PROD_CLS_MTRX_KEY,  CA.CUST_ACCT_KEY,CA.PROD_SRVC_KEY
...
SELECT PRM.PRMS_KEY, PRM.MO_END, PRM.MO_NM,PRM.REGION,
       ...
       CUST_ADDR.FIVE_DGT_ZIP,CUST_ADDR.CITY_NM,CUST_ADDR.ST_CD
...
FROM TMP_DATES da
     INNER JOIN TMP_SEG_TEN st
       ON st.SEG_CNCT_DAY_KEY <= da.END_DAY_KEY
       AND st.SEG_DISCNCT_DAY_KEY > da.END_DAY_KEY
...
SELECT PRMS_KEY, MO_NM,REGION, LVL_3_NM, CUST.ORG_KEY,
       ...
       CASE WHEN CNCT.CUST_ACCT_KEY IS NOT NULL
            THEN 'Connect'
            WHEN DISCNCT.CUST_ACCT_KEY IS NOT NULL
            THEN 'Disconnect'
...
FROM TMP_CUSTOMER CUST
     ...
     LEFT JOIN TMP_SEG_TEN_DISCONNECT DISCNCT
